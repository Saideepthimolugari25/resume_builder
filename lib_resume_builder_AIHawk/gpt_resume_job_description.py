...
from lib_resume_builder_AIHawk.gpt import AIAdapter
...
class LLMResumeJobDescription:
    ...
    def set_job_description_from_url(self, url_job_description):
        ...
        qa_chain = ({"context": context_formatter, "question": question_passthrough}
                    | ChatPromptTemplate.from_template(self.strings.summarize_prompt_template)
                    | self.llm_cheap | StrOutputParser())
    ...
    def generate_section(self, template: str, input_data: Dict[str, any]) -> str:
        prompt = ChatPromptTemplate.from_template(self._preprocess_template_string(template))
        chain = prompt | self.llm_cheap | StrOutputParser()
        return chain.invoke(input_data)

    def generate_html_resume(self) -> str:
        functions = {
            "header": lambda: self.generate_section(self.strings.prompt_header, {
                "personal_information": self.resume.personal_information,
                "job_description": self.job_description}),
            "education": lambda: self.generate_section(self.strings.prompt_education, {
                "education_details": self.resume.education_details,
                "job_description": self.job_description}),
            "work_experience": lambda: self.generate_section(self.strings.prompt_working_experience, {
                "experience_details": self.resume.experience_details,
                "job_description": self.job_description}),
            "side_projects": lambda: self.generate_section(self.strings.prompt_side_projects, {
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for future in as_completed(future_to_section):
                ...
                try:
                    ...
                    logger.debug(f'{section} generated an exception: {exc}')

        full_resume = "<body>\n" + f"  {results.get('header', '')}\n" + "  <main>\n"
        full_resume += "".join(f"    {results.get(section, '')}\n" for section in functions.keys() if section != 'header')
        full_resume += "  </main>\n</body>"
        return full_resume
